    ...
    async run(Client, msg, args) {
        ...
        if(Client.guilds.cache.get(guild) != undefined){
            ...
            const embed = new discord.MessageEmbed()
                ...
                {name:"Channels",value:`> ${Client.guilds.cache.get("869470072106803242").emojis.cache.get("872452332661661706")} ${text} ${Client.guilds.cache.get("869470072106803242").emojis.cache.get("872452348302204978")} ${voice}`,inline:true},
                {name:"😸・Emoji Count", value:`> ${Client.guilds.cache.get(guild).emojis.cache.size}`,inline:true},
                {name:"🗺️・Reigon", value:`> ${Client.guilds.cache.get(guild).region}`,inline:true})
